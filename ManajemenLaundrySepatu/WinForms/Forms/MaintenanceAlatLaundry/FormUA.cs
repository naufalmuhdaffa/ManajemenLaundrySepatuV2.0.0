            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    using (SqlTransaction transaction = conn.BeginTransaction())
                    {
                        try
                        {
                            using (SqlCommand cmd = new SqlCommand("sp_UpdateMaintenanceAlatLaundry", conn, transaction))
                            {
                                cmd.CommandType = CommandType.StoredProcedure;
                        ...
                        {
                            transaction.Rollback();
                            DarkModeMessageBox.Show("Terjadi kesalahan tidak terduga saat mengupdate mainenance alat laundry. Silakan coba lagi atau hubungi admin.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                }
                catch (SqlException sqlEx)
